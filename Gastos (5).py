...
def user_input_features():
  ...
  tiempo = st.number_input('Tiempo invertido (minutos)', min_value=1, max_value=300, value = 1, step = 1)
  tipo = st.number_input('Tipo',min_value=1, max_value=6, value = 1, step = 1)
  momento = st.number_input('Momento:', min_value=1, max_value=3, value = 1, step = 1)
  num_de_personas = st.number_input('No. de personas', min_value=1, max_value=50, value = 1, step = 1)

  user_input_data = {'Presupuesto': Presupuesto,
                     'Tiempo invertido (minutos)': tiempo,
                     'Tipo': tipo,
                     'Momento': momento,
                     'No. de personas': num_de_personas}
...
X = gasto.drop(columns=['Costo'], errors = 'ignore')
Y = gasto['Costo']
